...
const Register = () => {
  ...
  const getPendings = () => {
    ...
    axios
      ...
      .then((res) => {
        console.log('pendings:', res.data.pendings)
    ...
    <>
      <CModal size="lg" visible={isModal} onDismiss={() => setIsModal(false)} alignment="center">
        ...
        <CModalBody className="d-flex flex-column justify-content-center align-items-center h3">
          ...
          <br />
          line ID：{modalPerson.account}
          <br />
          信箱：{modalPerson.email}
          <br />
          手機：{modalPerson.cellphone}
          <br />
          <img src={modalPerson.imgSrc} alt="" className="img-fluid h-100 mt-1" />
        </CModalBody>
        <CModalFooter>
      ...
      <div className="auth-register mx-auto w-100 py-3">
        {applicants.length ? (
          <>
