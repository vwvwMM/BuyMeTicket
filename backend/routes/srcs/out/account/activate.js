...
const main = async (req, res) => {
  try {
    ...
    if (!doc) return res.send(template('activation code expired', `/register`))
    ...
    //register
    const { username, userpsw, email, cellphone } = doc
    const { _id: visual } = await new Visual({
      ...
      .save()
      .catch(dbCatch)
    await new Login({ username, account, email, cellphone, userpsw, visual })
      .save()
      .catch(async (e) => {
