...
const forget = async (req, res, next) => {
  ...
  //寄信
  const hylink = `${req.protocol}://${process.env.WEB_DOMAIN}/reset_password/${account}/${randomNum}`
  const hy_br = `${req.protocol}://${process.env.WEB_DOMAIN}/<wbr>reset_password/<wbr>${account}/<wbr>${randomNum}`
